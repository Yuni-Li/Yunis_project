...
//void insertReachable(List list, PlaceId location);
//void insertByType(List list, TransportType type, PlaceId location);
...
{
	Map m = MapNew();
	for (ConnList node = m->connections[location]; node != NULL; node = node->next)
	{
		ListInsert(list, node->p);
	}	
	MapFree(m);
}
...
void insertByType(List list, TransportType type, PlaceId location)
{
	...
	for (ConnList node = m->connections[location]; node != NULL; node = node->next)
	{
		if (node->type == type) ListInsert(list, node->p);
	}	
	MapFree(m);
}
*/
